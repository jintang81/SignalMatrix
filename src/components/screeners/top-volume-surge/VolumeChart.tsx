...
import type { TopVolumeSurgeChartData } from "@/types";
...
export default function VolumeChart({ chart }: Props) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Last price label (red for bearish signal)
      const lastC = chart.close[ve - 1];
      if (lastC != null) {
        ctx.fillStyle = C_DN; ctx.font = `bold 9px ${FONT}`; ctx.textAlign = "right";
        ctx.fillText(
          "$" + (lastC >= 1000 ? lastC.toFixed(0) : lastC >= 100 ? lastC.toFixed(1) : lastC.toFixed(2)),
          W - PAD.right + 50, yP(lastC) - 5,
        );
      }
      ...
      // Highlight last 2 bars (surge) in orange-red
      ...
      // Vol MA line (orange)
      ctx.strokeStyle = C_ORANGE; ctx.lineWidth = 1.2;
      ctx.beginPath();
      let vmStarted = false;
      for (let i = vs; i < ve; i++) {
        const v = chart.vol_ma30[i];
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      drag.startS = viewRef.current.start; drag.startE = viewRef.current.end;
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      viewRef.current.start = ns; viewRef.current.end = ne;
      draw();
    };

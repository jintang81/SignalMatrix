...
export default function StockCard({ stock: s }: Props) {
  ...
  return (
    <div className="panel overflow-hidden flex flex-col transition-all duration-200 hover:-translate-y-0.5 hover:border-dn/40">
      {/* Top: ticker + price */}
      <div className="flex items-start justify-between px-4 pt-3 pb-2.5 border-b border-border/50">
        ...
        <div className="text-right">
          ...
          <p className="text-xs font-trading mt-0.5 text-up">
      ...
      <div className="grid grid-cols-4 border-b border-border/40">
        {[
          { value: s.vol_ratio.toFixed(1) + "x",  label: "今日放量",  color: "text-dn"     },
          { value: s.vol_ratio2.toFixed(1) + "x", label: "昨日放量",  color: "text-dn"     },
          { value: "+" + s.ytd_return.toFixed(1) + "%", label: "YTD收益", color: "text-up" },

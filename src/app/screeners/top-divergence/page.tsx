...
import type { TopDivScreenerResult, TopDivStock } from "@/types";
import SummaryStats from "@/components/screeners/top-divergence/SummaryStats";
import FilterBar, { type TopDivFilterMode } from "@/components/screeners/top-divergence/FilterBar";
import StockCard from "@/components/screeners/top-divergence/StockCard";

type PageState =
  | { status: "loading" }
  | { status: "error"; message: string }
  | { status: "ready"; data: TopDivScreenerResult };

const POLL_INTERVAL  = 5000;
const MAX_POLL_COUNT = 72; // 72 × 5s = 6 min timeout

export default function TopDivergencePage() {
  const [state, setState]           = useState<PageState>({ status: "loading" });
  const [filter, setFilter]         = useState<TopDivFilterMode>("all");
  ...
  const startPolling = useCallback(() => {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      try {
        const s = await fetchTopDivStatus();
        setScanStatus(s);
        if (s.status === "done") {
          stopPolling();
          try {
            const data = await fetchTopDivScreener();
  ...
  const filtered = useMemo<TopDivStock[]>(() => {
      ...
      <div className="panel p-4">
        <div className="flex items-start justify-between gap-3 mb-1">
          <div className="flex items-center gap-3">
            <p className="text-sm tracking-[0.12em] text-dn">TOP DIVERGENCE</p>
            <span className="tag tag-muted text-[9px]">MACD / RSI 顶背离</span>
          </div>
        ...
        <p className="text-xs text-muted/60 leading-relaxed">
          价格创新高而动量指标不创新高 — 识别 S&amp;P500 + NASDAQ-100 + 主要 ETF 中的潜在顶部反转做空/止盈信号。
          MACD 两顶间隔 20–100 根 · 红柱缩短 ≥ 20% · RSI 两顶间隔 10–30 根 · 第二顶 RSI &gt; 70（超买区）
        </p>

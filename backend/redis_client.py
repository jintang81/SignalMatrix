...
MA20_KEY_RESULT = "screener:ma20:result"
MA20_KEY_STATUS = "screener:ma20:status"


def get_ma20_result() -> dict | None:
    """Returns the last MA20 turning-point scan result, or None if not yet available."""
    raw = _get_redis().get(MA20_KEY_RESULT)
    if raw is None:
        return None
    return json.loads(raw) if isinstance(raw, str) else raw


def set_ma20_result(data: dict) -> None:
    _get_redis().setex(MA20_KEY_RESULT, TTL, json.dumps(data, ensure_ascii=False))


def get_ma20_status() -> dict:
    """Returns status dict; defaults to {"status": "idle"} if key missing."""
    raw = _get_redis().get(MA20_KEY_STATUS)
    if raw is None:
        return {"status": "idle"}
    return json.loads(raw) if isinstance(raw, str) else raw


def set_ma20_status(status: str, **extra) -> None:
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    payload: dict = {"status": status, "updated_at": now, **extra}
    if status == "running":
        payload["started_at"] = now
    _get_redis().setex(MA20_KEY_STATUS, TTL, json.dumps(payload))

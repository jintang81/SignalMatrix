"""
MACD 倒鸭嘴筛选器 — Inverted Duck Bill Screener（空头信号）

筛选条件：
  1. 股票池：S&P500 + NASDAQ-100 + 主要 ETF（约 556 只）
  2. 均线空头排列：MA5 < MA10 < MA20
  3. MACD 倒鸭嘴三段式形成（以最近死叉为起点）：
     Phase A  死叉后连续 3 根 MACD 柱递减（负柱扩张）
     Phase B  DIFF 回弹趋近 DEA（差值 / 价格 < 1.5%），但不金叉
     Phase C  再次向下，开口角度 > 25°，DEA/DIFF 变化比 ≤ 0.65（非平行下行）
  4. Phase B 结束距今 ≤ 3 根交易日（形成足够新鲜）
  5. 全程 DIFF < DEA（死叉后不再金叉），最近 5 根在 0 轴下方
"""
...
from redis_client import set_inverted_duck_daily_snapshot
...
def detect_inverted_duck_bill(diff, dea, hist, close, window=DUCK_WINDOW, verbose=False):
    """
    倒鸭嘴形态检测（0 轴下方，空头信号）。
    返回 (bool, detail_dict)
    """
    ...
    if death_cross_idx < 0:
        return reject(f"最近{window}根内无死叉")

    d = all_d[death_cross_idx:]
    s = all_s[death_cross_idx:]
    h = all_h[death_cross_idx:]
    c = all_c[death_cross_idx:]
    w = len(d)

    if verbose:
        print(f"    死叉位置: 距今{n-1-death_cross_idx}根，死叉后段{w}根")

    if w < 7:
        return reject(f"死叉后段仅{w}根，不足7根")

    # 死叉后全程不能金叉
    if np.any(d >= s):
        cross_bars = [i for i in range(w) if d[i] >= s[i]]
        return reject(f"死叉后在 i={cross_bars} 处出现金叉")

    # 最近 5 根必须在 0 轴下方
    if not (np.all(d[-5:] < 0) and np.all(s[-5:] < 0)):
        return reject(f"最近5根未全在0轴下方")

    # Phase A：死叉后找连续 3 根 hist 递减（负值扩大），取最近满足的位置
    phase_a_end = -1
    ...
    # Phase B：回弹段，hist 负值缩小，DIFF 曾趋近 DEA
    phase_b_start = phase_a_end + 1
    phase_b_end   = -1
    min_gap_ratio = np.inf

    for i in range(phase_b_start, w - 1):
        gap_ratio = abs(d[i] - s[i]) / c[i]
        min_gap_ratio = min(min_gap_ratio, gap_ratio)
        if h[i] > h[i-1]:  # hist 负值缩小 = 回弹中
            phase_b_end = i

    if phase_b_end < 0:
        return reject("Phase B未找到（无 hist负值缩小的回弹）")
    ...
    if min_gap_ratio > NEAR_THRESHOLD:
        return reject(f"回弹未趋近DEA: min_gap={min_gap_ratio*100:.3f}% > {NEAR_THRESHOLD*100}%")
    ...
    if not (h[w-1] < h[w-2] < 0):
        return reject(f"Phase C负柱未递增扩大: h[-1]={h[w-1]:.3f}, h[-2]={h[w-2]:.3f}")

    if d[w-1] >= d[phase_b_end]:
        return reject(f"DIFF未重新下行: {d[w-1]:.4f} >= {d[phase_b_end]:.4f}")
    ...
    # 开口角度：Phase C 整段 DEA 下行幅度 > DIFF 下行幅度（倒鸭嘴开口向下）
    price_ref = c[-1]
    if price_ref <= 0:
        return reject("价格无效")
    ...
    diff_change_c = d[-1] - d[phase_c_start]   # 负数（下行）
    dea_change_c  = s[-1] - s[phase_c_start]    # 负数（下行）
    open_gap      = dea_change_c - diff_change_c  # DEA比DIFF下行更慢，开口向下
    ...
    if ratio > 0.65:
        return reject(f"两线近平行下行: DEA/DIFF比={ratio:.2f} > 0.65")
...
def check_ma_bearish(close):
    """MA5 < MA10 < MA20 空头排列（最新一根K线）"""
...
def screen_ticker(ticker, verbose=False):
    ...
    try:
        ...
        if verbose: print(f"  [{ticker}] 检测倒鸭嘴...")
        is_duck, duck_detail = detect_inverted_duck_bill(diff, dea, hist, close, verbose=verbose)
        if not is_duck:
            return "no_duck"

        is_bear, ma5, ma10, ma20 = check_ma_bearish(close)
        if not is_bear:
            if verbose: print(f"  [{ticker}] ✗ 非空头排列: MA5={ma5}, MA10={ma10}, MA20={ma20}")
            return "no_ma"
...
def run_inverted_duck_scan() -> dict:
    """
    ...
    set_inverted_duck_daily_snapshot(date_str, snapshot_entries)
